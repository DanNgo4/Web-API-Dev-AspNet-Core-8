...
using InvoiceApp.IntegrationTests.Fixtures;
    ...
    public async Task SendInvoiceAsync_ReturnsSuccessAndCorrectContentType(string invoiceId)
    {
        ...
        var client = factory
            .WithWebHostBuilder(x =>
            {
                ...
                {
                    var emailSender = y.SingleOrDefault(z => z.ServiceType == typeof(IEmailSender));
                    if (emailSender is null)
                        return;

                    y.Remove(emailSender);
                    y.AddScoped(_ => mockEmailSender.Object);
                });
            })
            .CreateClient();
