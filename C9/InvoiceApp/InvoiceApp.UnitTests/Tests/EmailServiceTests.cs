...
public class EmailServiceTests
{
    ...
    public void GenerateInvoiceEmail_Should_Return_Email()
    {
        ...
        var service = new EmailService(_loggerMock.Object, _emailSenderMock.Object);

        // Act
        var (to, subject, body) = service.GenerateInvoiceEmail(invoice);
    ...
    public async Task SendEmailAsync_Should_Send_Email()
    {
        ...
        _emailSenderMock.Setup(x => x.SendEmailAsync(It.IsAny<string>(), 
                                                     It.IsAny<string>(), 
                                                     It.IsAny<string>()))
                        .Returns(Task.CompletedTask);

        _loggerMock.Setup(x => x.Log(It.IsAny<LogLevel>(),
                                     It.IsAny<EventId>(),
        ...
        // Assert
        _emailSenderMock.Verify(x => x.SendEmailAsync(It.IsAny<string>(),
                                                     It.IsAny<string>(),
                                                     It.IsAny<string>()),
                                                     Times.Once);
        ...
        _loggerMock.Verify(x => x.Log(It.IsAny<LogLevel>(),
                                      It.IsAny<EventId>(),
                                      It.Is<It.IsAnyType>((y, z) => y.ToString().Contains($"Sending email to {to} with subject {subject} and body {body}")),
                                      It.IsAny<Exception>(),
                                      (Func<It.IsAnyType, Exception?, string>)It.IsAny<object>()),
                                Times.Once);
        ...
        _loggerMock.Verify(x => x.Log(It.IsAny<LogLevel>(),
                                      It.IsAny<EventId>(),
                                      It.Is<It.IsAnyType>((y, z) => y.ToString().Contains($"Email sent to {to} with subject {subject}")),
                                      It.IsAny<Exception>(),
                                      (Func<It.IsAnyType, Exception?, string>)It.IsAny<object>()),
                                Times.Once);
    }
    ...
    public async Task SendEmailAsync_Should_Log_SmtpException()
    {
        ...
        await act.Should()
                 .ThrowAsync<SmtpException>()
                 .WithMessage("Test SmtpException");*/
        ...
        _loggerMock.Verify(x => x.Log(It.IsAny<LogLevel>(),
                                      It.IsAny<EventId>(),
                                      It.Is<It.IsAnyType>((y, z) => y.ToString().Contains($"Failed to send email to {to} with subject {subject}")),
