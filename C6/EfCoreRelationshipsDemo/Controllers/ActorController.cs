...
[ApiController]
public class ActorController(SampleDbContext context) : ControllerBase
{
    // GET: api/Actors
    [HttpGet]
    public async Task<ActionResult<IList<Actor>>> GetActors()
    {
    ...
    [HttpGet("{id}/movies")]
    public async Task<ActionResult<IList<Movie>>> GetMovies(Guid id)
    {
        if (context.Actors == null)
            return NotFound("Actors is null.");
